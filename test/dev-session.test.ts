...
describe('managed development session', () => {
  ...
  it('keeps development env semantics when the parent process starts in production', async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        'NODE_ENV=development\nVITE_LINK_ENV_SENTINEL=development-file\n',
      )
      await writeEnvironmentServerEntry(root, runtimeUrl)
...
async function writeEnvironmentServerEntry(root: string, runtimeUrl: string): Promise<void> {
  await writeFile(
    join(root, 'src/main.ts'),
    [
      "import { createServer } from 'node:http'",
      ...
      '      sentinel: process.env.VITE_LINK_ENV_SENTINEL,',
      '    }))',
      '  })',
      "  await new Promise<void>((resolvePromise) => server.listen(Number(process.env.PORT), '127.0.0.1', resolvePromise))",
      '  return { close: () => new Promise<void>((resolvePromise, reject) => server.close((error) => error ? reject(error) : resolvePromise())) }',
...
async function expectJsonResponse(port: number, expected: unknown): Promise<void> {
  const started = Date.now()
  let lastError: unknown
  while (Date.now() - started < 5_000) {
    try {
      ...
      lastError = new Error(
        `Expected ${JSON.stringify(expected)}, received ${JSON.stringify(body)}`,
      )
    } catch (error) {
      lastError = error
    }
  ...
  throw lastError ?? new Error(`Timed out waiting for ${JSON.stringify(expected)}`)
}
